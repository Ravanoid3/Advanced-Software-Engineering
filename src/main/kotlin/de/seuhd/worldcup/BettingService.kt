...
object BettingService {
    ...
    fun evaluateBonus(matches: List<Match>): Int {
        ...
        for (match in matches) {
            ...
            val hasExactScores = bet.predictedHomeScore != null && bet.predictedAwayScore != null
            val exactMatch = hasExactScores && bet.predictedHomeScore == home && bet.predictedAwayScore == away
    ...
    fun changeBet(bet: Bet) {
        ...
        if (!bets.containsKey(bet.matchId)) {
            throw IllegalArgumentException("No bet exists for matchId ${bet.matchId}")
        }
        bets[bet.matchId] = bet
        cachedResult = null
